...
namespace FDEV.Rules.Demo.Core.Logging.CONSOLIDATE_CONFIG.Assemblies
{
    abstract class AssemblyFinder
    {
        public abstract IReadOnlyList<AssemblyName> FindAssembliesContainingName(string nameToFind);

        protected static bool IsCaseInsensitiveMatch(string text, string textToFind) => text != null && text.ToLowerInvariant().Contains(textToFind.ToLowerInvariant());

        public static AssemblyFinder Auto()
        {
            try
            {
                // Need to check `Assembly.GetEntryAssembly()` first because `DependencyContext.Default` throws an exception when `Assembly.GetEntryAssembly()` returns null
        ...
        public static AssemblyFinder ForSource(ConfigurationAssemblySource configurationAssemblySource)
        {
            return configurationAssemblySource switch
            {
                ...
                ConfigurationAssemblySource.AlwaysScanDllFiles => new DllScanningAssemblyFinder(),_
                    => throw new ArgumentOutOfRangeException(nameof(configurationAssemblySource), configurationAssemblySource, null),
            };
        }

        public static AssemblyFinder ForDependencyContext(DependencyContext dependencyContext) => new DependencyContextAssemblyFinder(dependencyContext);
    }
}
