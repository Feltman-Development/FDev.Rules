...
namespace FDEV.Rules.Demo.Core.Logging.CONSOLIDATE_CONFIG.Config
{
    class ObjectArgumentValue : IConfigurationArgumentValue
    {
        ...
        public object ConvertTo(Type toType, ResolutionContext resolutionContext)
        {
            ...
            if (typeInfo.IsGenericType && typeInfo.GetGenericTypeDefinition() is Type genericType && genericType == typeof(Action<>))
            {
            ...
            if (toType.IsArray) return CreateArray();
            if (IsContainer(toType, out var elementType) && TryCreateContainer(out var result)) return result;
            ...
            bool TryCreateContainer(out object result)
            {
                ...
                if (toType.GetConstructor(Type.EmptyTypes) == null) return false;
