...
namespace FDEV.Rules.Demo.Core.Logging.CONSOLIDATE_CONFIG.Config
{
    class StringArgumentValue : IConfigurationArgumentValue
    {
        ...
        public object ConvertTo(Type toType, ResolutionContext resolutionContext)
        {
            ...
            if (toType == typeof(LoggingLevelSwitch)) { return resolutionContext.LookUpLevelSwitchByName(argumentValue); }

            if (toType.FullName == "Serilog.Expressions.LoggingFilterSwitch" || toType.FullName == "Serilog.Filters.Expressions.LoggingFilterSwitch")
            {
            ...
            {
                if (string.IsNullOrEmpty(argumentValue)) return null;
            ...
            if (toTypeInfo.IsEnum) return Enum.Parse(toType, argumentValue);
            ...
            if (convertor != null) return convertor(argumentValue);
            ...
            if ((toTypeInfo.IsInterface || toTypeInfo.IsAbstract) && !string.IsNullOrWhiteSpace(argumentValue))
            {
                ...
                if (TryParseStaticMemberAccessor(argumentValue, out var accessorTypeName, out var memberName))
                {
                    ...
                    var publicStaticFieldInfo = accessorType.GetTypeInfo().DeclaredFields.Where(x => x.Name == memberName).Where(x => x.IsPublic).FirstOrDefault(x => x.IsStatic);
                ...
                if (ctor == null) throw new InvalidOperationException($"A default constructor was not found on {type.FullName}.");
